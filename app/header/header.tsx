    ...
    <header className="sticky top-0 z-40 border-b border-brand-700 bg-brand-600 backdrop-blur">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-8">
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-full text-white/90 transition-colors hover:bg-white/10 hover:text-white md:hidden"
        >
      ...
      {mobileOpen && (
        <nav className="border-t border-brand-700 px-4 py-3 md:hidden">
          <NavList items={NAV_ITEMS} className="flex-col items-stretch gap-1" variant="dark" />
          {account ? (
            <div className="mt-2 flex flex-col gap-1 border-t border-brand-500 pt-2">
              <div className="px-3 py-1 text-xs font-medium uppercase tracking-wide text-white/70">
            ...
            <NavList items={SIGN_IN_ITEM} className="mt-1 flex-col items-stretch gap-1" variant="dark" />
          )}
        </nav>
      )}
    </header>
